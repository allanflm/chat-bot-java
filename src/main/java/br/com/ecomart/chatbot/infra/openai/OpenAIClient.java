package br.com.ecomart.chatbot.infra.openai;
...
public class OpenAIClient {
    ...
    public String enviarRequisicaoChatCompletion(DadosRequisicaoChatCompletion dados) {
        var messageRequest = MessageRequest.builder().role(ChatMessageRole.USER.value()).content(dados.promptUsuario()).build();

        if (this.threadId == null) {
            var threadRequest = ThreadRequest.builder().messages(Arrays.asList(messageRequest)).build();
        ...
        var runRequest = RunCreateRequest.builder().assistantId(assistantId).build();
        ...
        var respostaAssistente = mensagens.getData()
                .stream()
